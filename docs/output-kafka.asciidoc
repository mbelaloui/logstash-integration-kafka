...
| <<plugins-{type}s-{plugin}-message_headers_field>> |<<string,string>>|No
...
[id="plugins-{type}s-{plugin}-message_headers_field"]
===== `message_headers_field`

  * Value type is <<string,string>>
  * There is no default value for this setting.

A {logstash-ref}/field-references-deepdive.html[field reference] to a hash whose entries are each added to the message
as a Kafka header. Header names are the hash keys, header values are taken from the hash values.
This allows forwarding a set of headers that is not known in advance, for example to mirror the
headers of a consumed Kafka record captured by the kafka input's
`decorate_events => extended` mode:
[source,ruby]
----------------------------------
    message_headers_field => "[@metadata][kafka][headers]"
----------------------------------

Headers from this setting are appended after the ones from `message_headers`. Kafka allows several
headers with the same name, so a name present in both settings results in two headers on the record.
Entries with an empty key or a `nil` value, and values that are not scalar (nested hashes or arrays),
are skipped. If the referenced field is absent the event is sent without additional headers; if it
does not contain a hash, a warning is logged and the field is ignored.
